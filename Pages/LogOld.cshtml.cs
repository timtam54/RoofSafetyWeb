...
{
    public class LogOldModel : PageModel
    {
        private readonly dbcontext _context;
        public LogOldModel(dbcontext context)
        {
            _context = context;
        }
        ...
        private void GetData()
        {
            ...
            Equipment = (from ine in _context.InspEquip join eqt in _context.EquipType on ine.EquipTypeID equals eqt.id where ine.InspectionID == ID select new EquipmentLogExcel { Installer = ine.Installer, InspectionDue = "", WithdrawalDate = "", Status = "NA", InspEquipID = ine.id, Number = ine.id, EquipmentType_Desc = eqt.EquipTypeDesc, SerialNo = ine.SerialNo, Manufacturer = ine.Manufacturer, InspectionDate = ins.InspectionDate.ToString("dd-MMM-yyyy") }).ToList();
